...
/*   Updated: 2021/02/07 13:33:56 by Jraye            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (!(prime->parse->mlx = ft_calloc(10, sizeof(void *))))
		error_print(BAD_MALLOC);
	if (!(prime->parse->img = ft_calloc(10, sizeof(void *))))
		error_print(BAD_MALLOC);
	if (!(prime->parse->addr = ft_calloc(10, sizeof(char *))))
		error_print(BAD_MALLOC);
	if (!(prime->parse->img_w = ft_calloc(10, sizeof(int))))
		error_print(BAD_MALLOC);
	if (!(prime->parse->img_h = ft_calloc(10, sizeof(int))))
		error_print(BAD_MALLOC);
	while (n < 10)
	{

...
/*   Updated: 2021/02/07 13:49:03 by Jraye            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_obj(t_prime *prime, int i, int j)
{
	while (prime->map[i])
	{
		...
		while (prime->map[i][j])
		{
			...
			if (prime->map[i][j] == '2' || prime->map[i][j] == '3' ||
			prime->map[i][j] == '4' || prime->map[i][j] == '5')
				prime->num_sprites++;
			j++;
		}
		i++;
	...
	while (prime->map[i])
	{
		...
		while (prime->map[i][j])
		{
			...
			prime->map[i][j] == '4' || prime->map[i][j] == '5')
			{
				...
				if (prime->map[i][j] == '3')
					prime->sprite[k].tex_num = 5;
				if (prime->map[i][j] == '4')
					prime->sprite[k].tex_num = 6;
				if (prime->map[i][j] == '5')
					prime->sprite[k].tex_num = 7;
				k++;
			}
			j++;
		}
